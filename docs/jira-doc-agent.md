...
| `scripts/precheck.py` | 2단계. 기계적 검사(T1, T2, B1, B2, I1, I2, R4, A1, A2) |
| `scripts/apply.py` | 4단계. Claude 출력을 검증해 Jira에 반영. Jira에 쓰는 유일한 곳 |
...
2. 실행 로그의 1단계에서 `[collect] INNO-17: 수집 완료`가, 2단계에서 `[precheck] INNO-17 (Task): AC n개`가 보이면 Jira 읽기가 된 것이다.
3. 3단계(Claude)가 끝난 뒤 4단계 요약(Summary 탭)에 `INNO-17 | Task | review | 통과|보완 요청|보류`가 나오면 Jira에 반영된 것이다.
4. INNO-17을 열어 TL;DR 필드와 description의 결과 산출물 구역이 바뀌었고, 진행 배경과 예상 산출물은 그대로이며, `[ai-doc-agent]`로 시작하는 검수 comment가 달렸는지 확인한다.
